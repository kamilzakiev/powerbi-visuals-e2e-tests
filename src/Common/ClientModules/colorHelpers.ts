namespace ClientHelpers {
    export function convertColorToRgbColor(color: string): jsCommon.Color.RgbColor {
        ...
        if(!color) {
            throw "convertAnyFontSizeToPixel: color is empty";
        }

        if(!_.isString(color)) {
            throw "convertAnyFontSizeToPixel: color is not string";
        }

        if(color.toLowerCase() === "transparent") {
            return jsCommon.Color.parseColorString("rgba(0, 0, 0, 0)");
        }
    ...
    export function areColorsEqual(color1: string, color2: string) {
        return convertColorToHexString(color1).toUpperCase() === convertColorToHexString(color2).toUpperCase();
    }

    export function assertColorsMatch(actual: string, expected: string, invert?: boolean) {
        var rgbActual = jsCommon.Color.parseColorString(convertColorToHexString(actual));
        var rgbExpected = jsCommon.Color.parseColorString(convertColorToHexString(expected));
